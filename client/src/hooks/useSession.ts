...
function normalizeSummary(raw: Record<string, unknown>): SessionSummary {
  return {
    ...
    copingStrategies: (raw.copingStrategies || []) as string[],
    homeworkAssignments: (raw.homeworkAssignments || []) as string[],
    topicsDiscussed: (raw.topicsDiscussed || []) as string[],
    emotionalThemes: (raw.emotionalThemes || []) as string[],
    issuesIdentified: (raw.issuesIdentified || []) as string[],
    conversationAssessment: (raw.conversationAssessment || '') as string,
    emotionalJourney: (raw.emotionalJourney || '') as string,
    riskLevel: (raw.riskLevel || 'low') as 'low' | 'moderate' | 'elevated',
    suggestedFocusAreas: (raw.suggestedFocusAreas || []) as string[],
    techniquesUsed: (raw.techniquesUsed || []) as string[],
    clinicalImpression: raw.clinicalImpression as string | undefined,
    preliminaryDiagnosis: raw.preliminaryDiagnosis as string | undefined,
    recommendedActions: (raw.recommendedActions || []) as string[],
    wayForward: raw.wayForward as string | undefined,
    preMood: (raw.preMood || null) as MoodEntry | null,
    postMood: (raw.postMood || null) as MoodEntry | null,
